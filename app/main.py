...
@app.post(
    "/persons", response_model=schemas.Person, status_code=status.HTTP_201_CREATED
)
def create_person(person: schemas.PersonCreate, db: Session = Depends(database.get_db)):
    ...
    if db_person:
        raise HTTPException(
            status_code=400, detail="Person with this name already registered"
        )
    if not person.name:
...
def get_person_by_id(*, db: Session = Depends(database.get_db), person_id: int):
    ...
    if not db_person:
        raise HTTPException(
            status_code=404, detail="Person with this id does not exist"
        )
    return db_person
...
def create_task(
    *,
    ...
    if (task.completed and not task.enddate) or (task.enddate and not task.completed):
        raise HTTPException(
            status_code=400, detail="enddate and completed values are invalid!"
        )
