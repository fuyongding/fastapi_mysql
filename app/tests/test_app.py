...
@pytest.mark.parametrize("invalid_name", [
    "",
    "A" * 51,  # Exceeds the maximum character limit of 50
])
...
@pytest.mark.parametrize("person_data, expected_status_code", [
    ({"name": "Alice Smith"}, 200),
    ({"name": "John Doe"}, 400),  # duplicate case
])
...
# Parameterized test cases for creating tasks with invalid names (assert 400)
@pytest.mark.parametrize("invalid_name", [
    "",  # Empty string
...
@pytest.mark.parametrize("task_data, expected_status_code", [
    ({"name": "Task 1", "description": "A" * 61, "completed": False, "startdate": "2023-09-07", "enddate": None}, 400), 
    ({"name": "Task 2", "description": "B" * 61, "completed": True, "startdate": "2023-09-08", "enddate": "2023-09-10"}, 400),
])
